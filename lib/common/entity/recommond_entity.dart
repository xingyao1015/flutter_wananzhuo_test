...
class RecommondDataData {
	...
	int type;
	int userId;
	List<RecommondDataDatasTag> tags;
	String apkLink;
	String envelopePic;
	int chapterId;
	int superChapterId;
	...
	RecommondDataData.fromJson(Map<String, dynamic> json) {
		...
		userId = json['userId'];
		if (json['tags'] != null) {
			tags = new List<RecommondDataDatasTag>();(json['tags'] as List).forEach((v) { tags.add(new RecommondDataDatasTag.fromJson(v)); });
		}
...
class RecommondDataDatasTag {
	String name;
	String url;

	RecommondDataDatasTag({this.name, this.url});

	RecommondDataDatasTag.fromJson(Map<String, dynamic> json) {
		name = json['name'];
		url = json['url'];
	}
